...
from professor_full import build_full, trace_key, resolve_professor
import bookmarks
import usage_alert
...
#  Helpers
# ──────────────────────────────────────────────
from term_order import term_sort_key  # noqa: E402  (shared with precompute.py)
...
def _write_all(statements):
    ...
    bookmarks.add_bookmark needs this: it inserts the new bookmark and collapses
    any row the user still holds under a retired slug for the same professor, and
    a partial apply leaves their bookmark count wrong. The pool is not autocommit,
    so a single commit after the last statement is the transaction.

    Rolls back on failure and re-raises, like _write and unlike _chat_write — a
    failed bookmark write must surface to the caller. The stale-connection retry
    replays the whole list, which is safe because both statements are idempotent
    (INSERT ... ON CONFLICT DO NOTHING, and a DELETE by key).
    """
    def run():
        conn = get_db()
        cur = conn.cursor()
...
def professor_profile(slug):
    ...
    # Look up professor from catalog (slug, then name_key, then retired slug)
    prof = resolve_professor(slug, query_one)

    if not prof:
        return jsonify({"error": "Professor not found"}), 404

    # Every name_key use below this point is a TRACE lookup, so resolve the
    # TRACE-side key once (fuzzy-matched professors differ from prof["name_key"]).
    ...
    if is_authed:
        if trace_course_rows:
            ...
            for r in rating_dist_rows:
                ...
                if course_code not in rating_dist_by_course:
                    rating_dist_by_course[course_code] = {"count1": 0, "count2": 0, "count3": 0, "count4": 0, "count5": 0}
        ...
        overall_rows = query("""
            SELECT tc.display_name, ts.count_1, ts.count_2, ts.count_3, ts.count_4, ts.count_5
            FROM trace_scores ts
            JOIN trace_courses tc
              ON ts.course_id = tc.course_id
             AND ts.instructor_id = tc.instructor_id
...
def professor_reviews(slug):
    ...
    name_key = prof["name_key"]          # RMP reviews live under the RMP name
    trace_nk = trace_key(prof)           # TRACE comments may live under another
...
def courses_catalog():
    ...
    # Named columns, not SELECT *: `limit` goes up to 10,000 here, and search_text
    # (the code plus every historical title of the course) would be fetched for
    # every one of those rows to be thrown away. is_topics is not among them
    # because nothing in the list rendering uses it — a topics code already
    # arrives with avg_rating NULL, which is exactly how the list shows it.
    rows = query(f"""
...
def course_profile(code):
    ...
    # Look up course in catalog. SELECT * so a catalog built before is_topics
    # existed still serves (the column reads as absent, i.e. not a topics code).
